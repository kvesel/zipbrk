 ...
 *   CVE-2018-1000035  Heap buffer overflow in unzip <= 6.00 when processing
 *                  password-protected archives.  Reachable via -e: setting the
 *                  encrypt bit on a non-encrypted entry causes unzip to read
 *                  plaintext compressed data as a 12-byte encryption header,
 *                  triggering OOB reads during key initialisation.  Sharpened
 *                  by -xs (XOR CRC-32), which corrupts the verification byte
 *                  unzip checks while setting up the decryption stream.
 *
